...
if uploaded_file is not None:
    ...
    st.info("ℹ️ Using default CSV dataset")
...
try:
    ...
    model = None
    st.warning(f"⚠️ Model not found or incompatible ({e}). Predictions will be disabled.")
...
# Expanded max PAY value to 10 to avoid validation issues
PAY_0 = user_input_field("PAY_0 (Sep)", min_val=-2, max_val=10)
PAY_2 = user_input_field("PAY_2 (Aug)", min_val=-2, max_val=10)
PAY_3 = user_input_field("PAY_3 (Jul)", min_val=-2, max_val=10)
PAY_4 = user_input_field("PAY_4 (Jun)", min_val=-2, max_val=10)
PAY_5 = user_input_field("PAY_5 (May)", min_val=-2, max_val=10)
PAY_6 = user_input_field("PAY_6 (Apr)", min_val=-2, max_val=10)
...
with tab2:
    ...
    if st.button("Predict Default"):
        if model is None:
            st.error("❌ Model not found or incompatible.")
        else:
            try:
                input_array = np.array(input_data).reshape(1, -1)
                if np.isnan(input_array).any():
                    st.error("⚠️ Some fields are empty or invalid. Please check your inputs.")
                else:
                    ...
                    st.download_button("⬇️ Download Prediction as CSV", csv, "prediction.csv", "text/csv")

            except Exception as e:
                st.error(f"⚠️ Prediction failed due to incompatible model or data format: {e}")
    ...
    if st.button("Generate Random Example"):
        for key in random_example:
            if key == 'LIMIT_BAL':
                ...
                random_example[key] = random.randint(-2, 10)
